...
public class ProfileDataAccessService implements ProfileDao {
	...
	public List<Profile> selectAllProfiles() {
		...
		String sql = "" + "SELECT profile_id, " + "name, " + "email " + "FROM profiles";
	...
	public Optional<Profile> selectProfileById(UUID id) {
		...
		Profile profile = jdbcTemplate.queryForObject(sql, new Object[] { id }, (resultSet, i) -> {
	...
	public int insertProfile(UUID id, Profile profile) {
		...
		String sql = "" + "INSERT INTO profiles (profile_id, name, email) " + "VALUES (?, ?, ?)";

		return jdbcTemplate.update(sql, id, profile.getName(), profile.getEmail());
	}
